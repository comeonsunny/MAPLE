...
public:
	...
	static inline long long mulmod_real(long long a, long long b) {
		unsigned long long a_psv = a < 0 ? -a : a;
		unsigned long long b_psv = b < 0 ? -b : b;
		// use flag to log the sign of the result of a*b
		int flag = (a < 0) ^ (b < 0) ? -1 : 1;
		unsigned long long res = mulmod(a_psv, b_psv);
		return flag * res;
	}
    
	static inline long long mulmod(unsigned long long a, unsigned long long b) 
	{
		#if defined(NTL_LIB)
